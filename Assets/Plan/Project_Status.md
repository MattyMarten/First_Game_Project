...
**Current stage: Stage 6, Merchant — DONE. Merchant fully extracted from Shop/DeskTwo into its own room (MerchantRoomManager): 13 pedestals (3 Utility, 1 Backpack, 2 Charm, 3 Material, 4 Shop Decor) roll fresh every morning on DayPhaseSystem.OnDayAdvanced; Shop Decor pedestals are gated by DecorItemData.tierIndex/effectType ownership, checked against the new DecorStorage (owned-not-placed) plus existing DecorManager (placed) — DecorStorage didn't exist before this stage and was added as a Storage-room prerequisite. Data Stick is a session-agreed simplification from Room_Merchant.md's literal "14th dedicated pedestal": instead, after the 13 roll normally there's an independent 10% chance one of them gets overwritten by a Data Stick for the day. Purchases support buy-one and buy-all (fixed a real bug in the original single-purchase draft where a whole stack was handed over for one unit's price). Room has a flavor-line sign (MerchantRoomManager.flavorDialogueLines, reseeded from the old MerchantProfileData's dialogue pool) that changes on every purchase. Curated Data Stick pool currently holds just the one real locked recipe in the project (Basic Shovel T1) — Basic Shovel T2's stray isUnlockedByDefault:false was also fixed (unintentional, tier upgrades should flow through the Gear Upgrade Station, not a second Data Stick). Old ServiceDeskManager/ServiceVisitorSpawner/ServiceDeskUI merchant-visit code and the MerchantOffer interaction type are fully removed; MerchantProfileData itself is safe to delete now that its dialogue lines live in MerchantRoomManager. Next: Stage 7, Quest Board + Info Room.**     // [update this line each session]
...
- Merchant's Data Stick pedestal is NOT a dedicated 14th physical pedestal (deviates from Room_Merchant.md's literal text). Instead: 13 physical pedestals total, and after they roll normally each morning there's an independent 10% chance one of them (chosen at random) gets overwritten by a Data Stick instead of its rolled item. Every pedestal prefab must be able to display a DataStickItem generically, not just its "home" category's item type. This is intentional, not a shortcut — treat Room_Merchant.md's "14 pedestals" language as superseded by this note.
